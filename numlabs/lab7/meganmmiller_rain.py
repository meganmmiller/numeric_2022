...
def initial_conditions(u, h, v, ho): #added v
    """Set the initial condition values.
    """
    u.prev[:] = 0
    h.prev[:] = 0
    v.prev[:] = 0 #added in v
    h.prev[len(h.prev) // 2] = ho # "//" is for quotient division, it only keeps the integer.


def boundary_conditions(u_array, h_array, v_array, n_grid): #added v array
    """Set the boundary condition values.
    """
    u_array[0] = 0 #as no flow through the walls
    u_array[n_grid - 1] = 0 #as no flow through the walls
    h_array[0] = h_array[1] # because there is no flux at the walls
    h_array[n_grid-1] = h_array[n_grid-2] #because there is no flux at the walls
    v_array[0] = 0 #no velocity at the walls
    v_array[n_grid-1] = 0 #no velocity at the walls


def first_time_step(u, h, v, g, H, dt, dx, ho, gu, gh, n_grid,f): #added v and f
    ...
    v.now[1:n_grid-1]=0
    v.now[midpoint+1]=g*ho*f*dt**2/(2*dx)
    v.now[midpoint-1]=-g*ho*f*dt**2/(2*dx)

def leap_frog(u, h, v, dt, gu, gh, n_grid, f):
    ...
    for pt in np.arange(1, n_grid - 1):
        u.next[pt] = f*v.now[pt]*2*dt+u.prev[pt] - gu * (h.now[pt + 1] - h.now[pt - 1])
        h.next[pt] = h.prev[pt] - gh * (u.now[pt + 1] - u.now[pt - 1])
        v.next[pt] = v.prev[pt]-2*f*dt*u.now[pt]
...
#     n_time, n_grid = map(int, args)
    ...
    f=2*7.29E-5 *np.sin(np.radians(50)) # coriolis parameter [rad/s], calculated with earths rotation from Pond and Pickard (1983), at a latitude of 50 degrees
    ...
    first_time_step(u, h, v, g, H, dt, dx, ho, gu, gh, n_grid, f)
    ...
    for t in np.arange(2, n_time):
        ...
        leap_frog(u, h, v, dt, gu, gh, n_grid, f)
